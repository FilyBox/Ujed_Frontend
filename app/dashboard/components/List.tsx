...
const List: React.FC<ListProps> = ({ reports, onDataChange }) => {
  ...
  const handleDeparmentChange = async (
    ...
    try {
      const updatedDepartment = await updateReportDepartment(
        reportId,
        newDepartment,
      ...
      toast.error("No fue posible actualizar el departamento", {
        duration: 3000,
      });
    }
  };
              ...
              <div className='text-sm flex flex-col gap-2'>
                ...
                {report.images.length > 1 ? (
                  <div className='flex gap-2'>
                    <Chip
                      size='sm'
                      startContent={<FaPhotoVideo size={18} color='white' />}
                      classNames={{
          ...
          >
            <div className='flex flex-col gap-4'>
              <p className='text-sm text-gray-500'>
                Usuario:{" "}
                {report.user.name
                  ? report.user.name.length > 30
                    ? `${report.user.name.substring(0, 150)}...`
                    : report.user.name
                  : "Sin usuario"}
              </p>
